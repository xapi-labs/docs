...
<Info>
  CLI 是 Sandbox 基础设施的薄客户端：它调用报价、生命周期、执行、文件、端口、扩展和审计
  API。CLI 不实现 Agent loop、记忆、DAG 或多 Agent 调度；文中的 Agent/CI 场景只是调用示例。
</Info>
...
| `--regions a,b` | 允许区域 |
| `--min-runtime 24h` | 最短连续运行要求；筛选生命周期声明，不代表宿主绝不重启 |
