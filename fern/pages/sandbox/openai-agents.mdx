...
<Info>
  本页是客户端集成示例：OpenAI Agents SDK 负责模型循环和工具决策，`ai.xapi.to`
  提供模型协议，xAPI Sandbox 只提供隔离计算、生命周期、审计和计费。xAPI Gateway
  不托管 Agent 记忆、Prompt 或编排。参见[产品边界与能力模型](/sandbox/scope)。
</Info>
